...
<form  target="baocms_frm" action="<?php echo U('article/create');?>" method="post">
  ...
  <tr>
        <td class="lfTdBt">音乐播放连接：</td>
        <td class="rgTdBt"><input type="text" name="data[audio]" value="<?php echo (($detail["audio"])?($detail["audio"]):''); ?>" class="manageInput" />
		<code>必须带上http://还有必须是mp3格式才能播放。</code>
            </td>
        </tr>
  		<tr>
        <td class="lfTdBt">视频播放连接：</td>
        <td class="rgTdBt"><input type="text" name="data[video]" value="<?php echo (($detail["video"])?($detail["video"]):''); ?>" class="manageInput" />
		<code>必须带上http://还有必须mp4格式才能播放。</code>
            </td>
        </tr>
         ...
         <tr>
               <td class="lfTdBt">
                ...
                <td>
                    <div style="width: 300px;height: 100px; float: left;">
                        <input type="hidden" name="data[video_photo]" value="<?php echo ($detail["video_photo"]); ?>" id="video_data_photo" />
                        <input id="video_photo_file" name="video_photo_file" type="file" multiple="true" value="" />
                    </div>
                    <div style="width: 300px;height: 100px; float: left;">
                        <img id="video_photo_img" width="80" height="80"  src="__ROOT__/attachs/<?php echo (($detail["video_photo"])?($detail["video_photo"]):'default.jpg'); ?>" />
                        <a href="<?php echo U('setting/attachs');?>">设置</a>
                        建议尺寸<?php echo ($CONFIG["attachs"]["article"]["thumb"]); ?>
                    </div>
                    <script>
                        $("#video_photo_file").uploadify({
                            'swf': '__PUBLIC__/js/uploadify/uploadify.swf?t=<?php echo ($nowtime); ?>',
                            'uploader': '<?php echo U("app/upload/uploadify",array("model"=>"article"));?>',
                            'cancelImg': '__PUBLIC__/js/uploadify/uploadify-cancel.png',
                            'buttonText': '上传视频封面',
                            ...
                            'queueSizeLimit': 1,
                            'onUploadSuccess': function (file, data, response) {
                                $("#video_data_photo").val(data);
                                $("#video_photo_img").attr('src', '__ROOT__/attachs/' + data).show();
                            }
                        });
                    </script>
                </td>
