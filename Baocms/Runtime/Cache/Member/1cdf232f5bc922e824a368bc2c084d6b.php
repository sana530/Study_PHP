...
<div class="content_right">  
  ...
  <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><table class="tuan_table1" width="100%" border="0">
  <tbody><tr class="tr_left_2">
    <td class="left10" colspan="4">
    收藏编号：<span class="mallorder_jg"><?php echo ($item["favorites_id"]); ?></span>
    <span class="td_top_1">收藏时间：<?php echo (date('Y-m-d H:i',$item["create_time"])); ?>  </span></td>
  </tr>
  <tr>
    <td class="td_left_1"> 
    ...
    <table class="tuan_table2" width="100%" border="0">
      <tbody><tr class="tr_left_1">
        ...
        <a target="_blank" href="<?php echo ($shops[$item['shop_id']]['shop_name']); ?>" class="index__pic___TScfk"><img src="__ROOT__/attachs/<?php echo (($shops[$item['shop_id']]['logo'])?($shops[$item['shop_id']]['logo']):'default.jpg'); ?>"><span></span></a>
       ...
       <!--商品展示END-->
        ...
        <td class="left3"><?php echo ($shops[$item['shop_id']]['shop_name']); ?></td>
       
      </tr>
     
    </tbody></table>    
    </td>
    <td class="left7" width="15%">$<?php echo ($prices[$item['shop_id']]['price']); ?>
     </td>
    <td class="left8" width="20%">
    <?php echo round($shops[$item['shop_id']]['score']/10,1);?>

    </td>
    <td class="left9" width="15%">
   <a class="myInfor_sx" mini='confirm' href="<?php echo U('favorites/deletefavo',array('favorites_id'=>$item['favorites_id']));?>">取消关注</a>
                            <a class="myInfor_sx" href="<?php echo U('pchome/shop/detail',array('shop_id'=>$item['shop_id']));?>#huodong">查看活动</a>
    </td>
  </tr>
