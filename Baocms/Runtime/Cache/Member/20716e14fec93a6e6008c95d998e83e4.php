...
<div class="content_right"> 
	<p class="hotel_ts fontcl2">
            <?php if(!$detail['order_status']): ?>等待支付<small class="black9"></small><a mini='confirm' href="<?php echo U('member/booking/cancel',array('order_id'=>$detail['order_id']));?>" class="btn right">取消订单</a>
            <?php elseif($detail['order_status'] == 1): ?>
                订单已支付 
                    <a mini='confirm' href="<?php echo U('member/booking/cancel',array('order_id'=>$detail['order_id']));?>" class="btn right">取消订单</a>
            <?php elseif($detail['order_status'] == -1): ?>
                订单已取消
            <?php else: ?>
                订单已完成
                <?php if(($detail["comment_status"]) == "0"): ?><a href="<?php echo U('member/booking/comment',array('order_id'=>$detail['order_id']));?>" class="btn right">去评价</a>
                <?php else: ?>
                    已评价<?php endif; endif; ?>
        </p>
    <div class="hotel_orddtl_box mb20">
        ...
        <div class="hotel_orddtl_infor">
        	<ul>
        	    <li class="list">订单号：<span class="fontcl2"><?php echo ($detail["order_id"]); ?></span></li>
                <li class="list">商家名称：<span class="fontcl2"><?php echo ($shop["shop_name"]); ?></span></li>
                <li class="list">商家地址：<span class="fontcl2"><?php echo ($shop["addr"]); ?></span></li>
                <li class="list">预订时间：<span class="fontcl2"><?php echo ($detail["ding_date"]); ?>&nbsp;&nbsp;<?php echo ($detail["ding_time"]); ?></span></li>
                <li class="list">预计人数：<?php echo ($detail["ding_num"]); ?></li>
                <li class="list">点餐价格：<span class="fontcl2"><?php echo round($detail['menu_amount']/100,1);?>NZD(不需要支付)</span></li>
                <li class="list">定金：<span class="fontcl2"><?php echo round($detail['amount']/100,1);?>NZD</span></li>
    	    </ul>
        </div>
        <div class="hotel_orddtl_infor">
        	<ul>
                <li class="list">预订姓名：<span class="fontcl2"><?php echo ($detail["name"]); ?></span></li>
                <li class="list">手机号码：<span class="fontcl2"><?php echo ($detail["mobile"]); ?></span></li>
    	    </ul>
        </div>
        <div class="hotel_orddtl_infor">
        	<ul>
                <li class="list">补充说明：<span class="blackcl9"><?php echo (($detail["note"])?($detail["note"]):'无'); ?></span></li>
    	    </ul>
        </div>
    </div>
    <div class="hotel_orddtl_box mb20">
        ...
        <div class="hotel_orddtl_tabl mb20">
        	<table width="100%" border="0" cellspacing="0" cellpadding="0">
                ...
                <?php if(is_array($list)): foreach($list as $key=>$item): ?><tr>
                        <td>
                            <div class="tuw"><img src="<?php echo config_img($menus[$item['menu_id']]['photo']);?>" width="125" height="100"><?php echo ($item['menu_name']); ?></div>
                        </td>
                        <td><?php echo ($item["num"]); ?>份</td>
                        <td>$<?php echo round($item['price']/100,1);?></td>
                        <td><?php echo ($menus[$item['menu_id']]['sold_num']); ?>份</td>
                        <td><p class="price"><span class="fontcl2">$<big><?php echo round($item['amount']/100,1);?></big></span></p></td>
                    </tr><?php endforeach; endif; ?>
            </table>
        </div>
    </div>
        <?php if(!$detail['order_status'] and $detail['amount'] > 0): ?><div class="hotel_tongji">
            在线应支付<span class="price">$<?php echo round($detail['amount']/100,1);?></span>
            <a href="<?php echo U('home/booking/pay',array('order_id'=>$detail['order_id']));?>" class="btn">去支付</a>
        </div><?php endif; ?>
</div>
<!--左侧内容end-->
</div>
