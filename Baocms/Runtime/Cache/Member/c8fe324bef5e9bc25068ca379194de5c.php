...
<script>
    ...
    <div class="navList">
        ...
        <?php else: ?>
       		<li class="navListAll"><span class="navListAllt">全部抢购分类</span><div class="shadowy navAll"><link href="http://kevinpay/themes/default/Pchome/statics/css/style.css" rel="stylesheet" type="text/css" />
...
<div class="tgdd">
    ...
    <div class="tgdd_t">
        ...
        <form action="<?php echo U('ele/index');?>" method="post">
  ...
  <tbody><tr class="tr_left_2">
    ...
    <span class="td_top_1">下单时间：<?php echo (date('Y-m-d H:i:s',$order["create_time"])); ?>   </span></td>
  </tr>
  <tr>
    <td class="td_left_1"> 
    ...
    <table class="tuan_table2" width="100%" border="0">
      <tbody><tr class="tr_left_1">
        ...
        <a target="_blank" href="<?php echo U('pchome/shop/detail',array('shop_id'=>$order['shop_id']));?>" class="index__pic___TScfk"><img src="__ROOT__/attachs/<?php echo ($shop_s[$order['shop_id']]['photo']); ?>"><span></span></a>
        <div class="index__infos___A6XLq">
            <p><a href="<?php echo U('pchome/shop/detail',array('shop_id'=>$order['shop_id']));?>" target="_blank"><span></span><span><?php echo ($shop_s[$order['shop_id']]['shop_name']); ?></span><span></span></a><span></span><span></span></p>
            <span></span>
       ...
       <!--商品展示END-->
        ...
        <td class="left2">
         <?php if(is_array($products)): foreach($products as $key=>$product): if($product['order_id'] == $order['order_id']): ?><li class="mealOrder_nrli"><?php echo ($eleproducts[$product['product_id']]['product_name']); ?>（<?php echo ($product["num"]); ?>份）</li><?php endif; endforeach; endif; ?>
        </td>
        <td class="left3">$<?php echo round($order['total_price']/100,2);?></td>
        <td class="left5"> </td>
      </tr>
    ...
    <?php if(!empty($areas[$addrs[$order['addr_id']]['area_id']]['area_name'])): ?>地区：<?php echo ($areas[$addrs[$order['addr_id']]['area_id']]['area_name']); ?><br/><?php endif; ?>
       <?php if(!empty($bizs[$addrs[$order['addr_id']]['business_id']]['business_name'])): ?>商圈：<?php echo ($bizs[$addrs[$order['addr_id']]['business_id']]['business_name']); ?><br/><?php endif; ?>
       <?php if(!empty($addrs[$order['addr_id']]['addr'])): ?>地址： <?php echo ($addrs[$order['addr_id']]['addr']); ?><br/><?php endif; ?>
       <?php if(!empty($addrs[$order['addr_id']]['name'])): ?>姓名： <?php echo ($addrs[$order['addr_id']]['name']); ?><br/><?php endif; ?>
       <?php if(!empty($addrs[$order['addr_id']]['mobile'])): ?>电话： <?php echo ($addrs[$order['addr_id']]['mobile']); endif; ?>
     </td>
    <td class="left8" width="20%">
   <?php if(($order["is_pay"]) == "1"): if(($order["status"]) == "0"): ?><a href="<?php echo U('pchome/ele/pay',array('order_id'=>$order['order_id']));?>" target="_blank">去付款</a>
       <?php else: ?>
      	 <a > <?php echo ($cfg[$order['status']]); ?></a><?php endif; ?>
       <?php else: ?>
       	<?php if(($order["status"]) == "0"): ?><a href="<?php echo U('pchome/ele/pay',array('order_id'=>$order['order_id']));?>" target="_blank">去付款</a>
      	 <?php else: ?>
       餐到付款&nbsp;&nbsp;<?php echo ($cfg[$order['status']]); endif; endif; ?>   

    </td>
    <td class="left9" width="15%">
      
                <?php if(($order["status"]) == "8"): ?><a mini='confirm' href="<?php echo U('member/ele/delete',array('order_id'=>$order['order_id']));?>">删除</a><?php endif; ?>
                       <?php if(($order["status"]) == "0"): ?><a mini='confirm' href="<?php echo U('member/ele/delete',array('order_id'=>$order['order_id']));?>">删除</a><?php endif; ?>
                         <?php if(($order["status"]) == "4"): ?><a mini='confirm' href="<?php echo U('member/ele/delete',array('order_id'=>$order['order_id']));?>">删除</a><?php endif; ?>
                       <?php if(($order["is_pay"]) == "1"): if(($order["status"]) == "1"): ?><a mini='confirm'href="<?php echo U('member/ele/elecancle',array('order_id'=>$order['order_id']));?>">申请退款</a></if><?php endif; ?>
                      <?php if(($order["status"]) == "3"): ?><a mini='confirm' href="<?php echo U('member/ele/eleqxtk',array('order_id'=>$order['order_id']));?>">取消退款</a><?php endif; endif; ?>
                      <?php if(($order["status"]) == "8"): if($order['dianping'] == 0): ?><a href="<?php echo U('ele/dianping',array('order_id'=>$order['order_id']));?>">点评</a>
                      <?php else: ?>
                      <a href="javascript:void(0);">已点评</a><?php endif; endif; ?>
              
         <?php if($order['status'] == 2): ?><a mini='confirm' href="<?php echo U('member/ele/yes',array('order_id'=>$order['order_id']));?>">确认收货</a><?php endif; ?>
