...
.text-area { width: 100%;border: thin solid #eee;resize: none; margin-top:10px; border-radius:3px;}
	...
	<form class="pay-form" action="<?php echo U('mall/pay2',array('order_id'=>$order['order_id']));?>" method="post" target="x-frame">
		<div class="row">
			...
			<span class="float-right">$ <?php echo round($order['total_price']/100,2);?> NZD</span>
		</div>
		<hr />
        <!--使用优惠劵逻辑开始-->
        <?php if(!empty($coupon)): ?><div class="row">
                <span class="float-left">优惠劵使用:</span>
                <input type="hidden" name="download_id" id="download_id" value="<?php echo ($coupon['download_id']); ?>" checked="checked" />
                <span class="float-right"> 优惠劵ID:<?php echo ($coupon['download_id']); ?>，抵现 &#36;<?php echo round($coupon['reduce_price']/100,2);?>NZD</span>
            </div>
            <hr />
