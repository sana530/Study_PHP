...
<div class="pagewd">
    <div class="seat_banner mt10">
        ...
        <div class="flexslider seat_flexslider">
            <ul class="slides">
                <?php  $cache = cache(array('type'=>'File','expire'=> 21600)); $token = md5("Ad, bg_date <= '{$today}' AND  city_id IN ({$city_ids}) AND end_date >= '{$today}' AND closed=0 AND site_id=15,0,3,21600,orderby asc,,"); if(!$items= $cache->get($token)){ $items = D("Ad")->where(" bg_date <= '{$today}' AND  city_id IN ({$city_ids}) AND end_date >= '{$today}' AND closed=0 AND site_id=15")->order("orderby asc")->limit("0,3")->select(); $cache->set($token,$items); } ; $index=0; foreach($items as $item): $index++; ?><li class="list"><a target="_blank" href="<?php echo ($item["link_url"]); ?>"><img src="<?php echo config_img($item['photo']);?>" width="1200" height="360" /></a></li> <?php endforeach; ?>
        ...
        <div class="seat_form_box">
            <form action="<?php echo U('booking/lists');?>" method="post">
                ...
                <div class="short_box">
                    ...
                    <input class="long date left" type="text" name="date" value="<?php echo ($ding_date); ?>" onfocus="WdatePicker({dateFmt: 'yyyy-MM-dd',minDate:'%y-%M-{%d}'});" placeholder="日期" />
            ...
            <div class="style"> 
                <p>您还可以选择以下就餐类型：</p>
                <?php if(is_array($dingtypes)): $i = 0; $__LIST__ = $dingtypes;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i; if($i <= 3): ?><a href="<?php echo LinkTo('booking/lists',array('type_id'=>$i));?>" ><?php echo ($item); ?></a><?php endif; endforeach; endif; else: echo "" ;endif; ?>
            </div>
        </div>
        <!--表单end-->
    </div>
	...
	<div class="seat_cateSelct_cont">
             <div class="list_box seat_goodsList_box">
                <ul>
                    <?php  $cache = cache(array('type'=>'File','expire'=> 21600)); $token = md5("Booking,audit = 1 AND closed=0 AND city_id=$city_id,0,20,21600,orders desc,,"); if(!$items= $cache->get($token)){ $items = D("Booking")->where("audit = 1 AND closed=0 AND city_id=$city_id")->order("orders desc")->limit("0,20")->select(); $cache->set($token,$items); } ; $index=0; foreach($items as $item): $index++; $item['d'] = getDistanceCN($lat, $lng, $item['lat'], $item['lng']); ?>
                        <li class="seat_goodsList">
                            <div class="pub_img"><a target="_blank" href="<?php echo U('booking/detail',array('shop_id'=>$item['shop_id']));?>">
                                <img src="<?php echo config_img($item['photo']);?>" />
                                <p><?php echo bao_msubstr($item[addr],0,15);?></p></a>
                                <span class="intro2"><?php echo ($item["d"]); ?></span>
                            </div>
                            <div class="pub_wz">
                                <h3 class="overflow_clear"><a target="_blank" href="<?php echo U('booking/detail',array('shop_id'=>$item['shop_id']));?>"><?php echo ($item["shop_name"]); ?></a></h3>
                                <p class="mt10 mb10">口味：<span class="fontcl1 mr10"><?php echo ($item['kw_score']); ?></span>环境：<span class="fontcl1 mr10"><?php echo ($item['hj_score']); ?></span>服务：<span class="fontcl1 mr10"><?php echo ($item['fw_score']); ?></span></p>
                                <div class="btn_box">
                                    <div class="left">人均：<span class="fontcl1">$<?php echo ($item["price"]); ?></span></div>
                                    <a target="_blank" href="<?php echo U('booking/detail',array('shop_id'=>$item['shop_id']));?>" class="btn right">立即预订</a>
                                </div>
                            </div>
                        </li> <?php endforeach; ?>
                </ul>
        ...
        <div class="list_box seat_goodsList_box" style="display:none;">
            <ul>
            	<?php  $cache = cache(array('type'=>'File','expire'=> 10800)); $token = md5("Booking,audit = 1 AND closed=0 AND city_id=$city_id,0,4,10800,kw_score desc,,"); if(!$items= $cache->get($token)){ $items = D("Booking")->where("audit = 1 AND closed=0 AND city_id=$city_id")->order("kw_score desc")->limit("0,4")->select(); $cache->set($token,$items); } ; $index=0; foreach($items as $item): $index++; $item['d'] = getDistanceCN($lat, $lng, $item['lat'], $item['lng']); ?>
                    <li class="seat_goodsList">
                        <div class="pub_img"><a target="_blank" href="<?php echo U('booking/detail',array('shop_id'=>$item['shop_id']));?>">
                            <img src="<?php echo config_img($item['photo']);?>" />
                            <p><?php echo bao_msubstr($item[addr],0,15);?></p></a>
        ...
        <div class="list_box seat_goodsList_box" style="display:none;">
            <ul>
            	<?php  $cache = cache(array('type'=>'File','expire'=> 21600)); $token = md5("Booking,audit = 1 AND closed=0 AND city_id=$city_id,0,4,21600,score desc,,"); if(!$items= $cache->get($token)){ $items = D("Booking")->where("audit = 1 AND closed=0 AND city_id=$city_id")->order("score desc")->limit("0,4")->select(); $cache->set($token,$items); } ; $index=0; foreach($items as $item): $index++; $item['d'] = getDistanceCN($lat, $lng, $item['lat'], $item['lng']); ?>
                    <li class="seat_goodsList">
                        <div class="pub_img"><a target="_blank" href="<?php echo U('booking/detail',array('shop_id'=>$item['shop_id']));?>"><img src="<?php echo config_img($item['photo']);?>" />
                            <p><?php echo bao_msubstr($item[addr],0,15);?></p></a>
                            <span class="intro2"><?php echo ($item["d"]); ?></span>
                        </div>
                        <div class="pub_wz">
        ...
        <div class="list_box seat_goodsList_box" style="display:none;">
            <ul>
            	<?php  $cache = cache(array('type'=>'File','expire'=> 21600)); $token = md5("Booking,audit = 1 AND closed=0 AND city_id=$city_id,0,4,21600,hj_score desc,fw_score desc,,"); if(!$items= $cache->get($token)){ $items = D("Booking")->where("audit = 1 AND closed=0 AND city_id=$city_id")->order("hj_score desc,fw_score desc")->limit("0,4")->select(); $cache->set($token,$items); } ; $index=0; foreach($items as $item): $index++; $item['d'] = getDistanceCN($lat, $lng, $item['lat'], $item['lng']); ?>
                    <li class="seat_goodsList">
                        <div class="pub_img"><a target="_blank" href="<?php echo U('booking/detail',array('shop_id'=>$item['shop_id']));?>">
                            <img src="<?php echo config_img($item['photo']);?>" />
                            <p><?php echo bao_msubstr($item[addr],0,15);?></p></a>
    ...
    <!--筛选内容end-->
    <a target="_blank" href="<?php echo LinkTo('booking/lists');?>" class="seat_goodsList_more">更多优惠订座，请点击查看 &gt;</a>
</div>
<script>
$(document).ready(function () {
