...
<style>.intl-tel-input * { box-sizing: unset; }</style>
<div class="between">
   ...
   <div class="clear"></div>
        ...
        <div class="mode_dingdan">
            <form method="post" id="ele_form">
                <?php if($order['is_delivery'] == 1): if(!empty($useraddr)): ?><div class="order-addres">
                        ...
                        <div class="box">
                            ...
                            <div class="addright_on">
                                <div class="left">
                                    <?php $item = $useraddr[0]; ?>
                                    <label><input type="radio" name="addr_id" id="addr_id_<?php echo ($item["addr_id"]); ?>" <?php if(($item["addr_id"]) == $item["addr_id"]): ?>checked="checked"<?php endif; ?> value="<?php echo ($item["addr_id"]); ?>"/><span><?php echo ($citys[$item['city_id']]['name']); ?>&nbsp;&nbsp;<?php echo ($areas[$item['area_id']]['area_name']); ?>&nbsp;&nbsp;<?php echo ($bizs[$item['business_id']]['business_name']); ?>&nbsp;&nbsp;<?php echo ($item["addr"]); ?>（<?php echo ($item["name"]); ?>收）<?php echo ($item["mobile"]); ?></span></label>
                                </div>
                                <div class="right "><a href="javascript:void(0);" class="order-mr">使用地址</a> <span style="display:none;" id="addr_idss"><?php echo ($item["addr_id"]); ?></span><span style="display:none;" id="city_idss"><?php echo ($citys[$item['city_id']]['name']); ?></span><span style="display:none;" id="area_idss"><?php echo ($areas[$item['area_id']]['area_name']); ?></span><span style="display:none;" id="business_idss"><?php echo ($bizs[$item['business_id']]['business_name']); ?></span><span style="display:none;" id="addrss"><?php echo ($item["addr"]); ?></span><span style="display:none;" id="mobiless"><?php echo ($item['mobile']); ?></span><span style="display:none;" id="namess"><?php echo ($item['name']); ?></span> <a href="javascript:void(0);" c="<?php echo ($citys[$item['city_id']]['city_id']); ?>" b="<?php echo ($bizs[$item['business_id']]['business_id']); ?>" a="<?php echo ($areas[$item['area_id']]['area_id']); ?>"  class="order-xg">修改地址</a></div>
                            </div>
                            <?php $i=0 ?>
                            <?php if(is_array($useraddr)): foreach($useraddr as $key=>$item): $i++;if($i>1){ ?>
                                <div class="addright none">
                                    <div class="left"><label><input type="radio" name="addr_id" id="addr_id_<?php echo ($item["addr_id"]); ?>"  value="<?php echo ($item["addr_id"]); ?>"/><span rel="<?php echo ($item["addr_id"]); ?>" class="order-mr-change"><?php echo ($citys[$item['city_id']]['name']); ?>&nbsp;&nbsp;<?php echo ($areas[$item['area_id']]['area_name']); ?>&nbsp;&nbsp;<?php echo ($bizs[$item['business_id']]['business_name']); ?>&nbsp;&nbsp;<?php echo ($item["addr"]); ?>（<?php echo ($item["name"]); ?>收）<?php echo ($item["mobile"]); ?></span></label>
                                    </div>
                                    <div class="right addrightLink none">
                                        <a href="javascript:void(0);" rel="<?php echo ($item["addr_id"]); ?>" class="order-mr order-mr-change">更换地址</a>
                                        <span style="display:none;" id="addr_idss"><?php echo ($item["addr_id"]); ?></span>
                                        <span style="display:none;" id="city_idss"><?php echo ($citys[$item['city_id']]['name']); ?></span>
                                        <span style="display:none;" id="area_idss"><?php echo ($areas[$item['area_id']]['area_name']); ?></span>
                                        <span style="display:none;" id="business_idss"><?php echo ($bizs[$item['business_id']]['business_name']); ?></span>
                                        <span style="display:none;" id="addrss"><?php echo ($item["addr"]); ?></span>
                                        <span style="display:none;" id="mobiless"><?php echo ($item['mobile']); ?></span>
                                        <span style="display:none;" id="namess"><?php echo ($item['name']); ?></span>
                                    </div>
                                </div>
                ...
                <?php else: ?>
                    <div class="order-addres">
                        <h3 style="border-bottom: none;"><span class="left addlt">请到店用餐或自行取餐</span></h3>
                    </div><?php endif; ?>
                <div id="distance"></div>
                <div class="mode">选择支付方式</div>
                <ul class="mode_zx">
                    <li class="on">在线付款</li>
                    <?php if($order['is_delivery'] == 0): ?><li>现金支付</li><?php endif; ?>
                </ul>
                <div class="order_type table1">
                    <div  class="order_style">
                        ...
                        <script>
                            $(document).ready(function () {
                                if ("<?php echo ($order['is_delivery']); ?>" == 1) {
                                    distance("<?php echo ($order["order_id"]); ?>", "<?php echo ($useraddr[0]['addr']); ?>"+', '+"<?php echo ($bizs[$useraddr[0]['business_id']]['business_name']); ?>"+', '+"<?php echo ($areas[$useraddr[0]['area_id']]['area_name']); ?>"+', '+"<?php echo ($citys[$useraddr[0]['city_id']]['name']); ?>");
                                }
                                ...
                                $(".box").on('click','.order-mr-change',function () {
                                    ...
                                    $.post("<?php echo U('ele/change_addr');?>", {order_id: order_id, addr_id: addr_idd}, function (data) {
                                        if (data.status == 'success') {
                                            var content = '';
                                            datas = data.res;
                                            $.each(datas, function (n, value) {
                                                if (n == 0) {
                                                    ...
                                                    content += '<a href="javascript:void(0);" c="' + value.city_id + '" b="' + value.business_id + '" a="' + value.area_id + '"  class="order-xg">修改地址</a>';
                                                    content += '</div></div>';
                                                    $('.addrs_id').val('value.addr_id');
                                                } else {
                                                    ...
                                                    content += '<span rel="'+value.addr_id+'" class="order-mr-change">' + value.city + '&nbsp;&nbsp;' + value.area + '&nbsp;&nbsp;' + value.bizs + '&nbsp;&nbsp;' + value.addr + '（' + value.name + '收）' + value.mobile + '</span></label></div>';
                                                    content += ' <div class="right"><a rel="'+value.addr_id+'" href="javascript:void(0);" class="order-mr order-mr-change">更换地址</a>';
                                                    content += '<span id="addr_idss" style="display:none;">' + value.addr_id + '</span><span id="city_idss" style="display:none;">' + value.city_id + '</span><span id="area_idss" style="display:none;">' + value.area_id + '</span><span id="business_idss" style="display:none;">' + value.business_id + '</span><span id="namess" style="display:none;">' + value.name + '</span><span id="addrss" style="display:none;">' + value.addr + '</span><span id="mobiless" style="display:none;">' + value.mobile + '</span>';
                                                    content += '</div></div>';
                                                }
                                            });
                                            $('.box').html(content);
                                            $(".snew").show();
                                            distance("<?php echo ($order["order_id"]); ?>", datas[0]['addr']+', '+datas[0]['bizs']+', '+datas[0]['area']+', '+datas[0]['city']);
                            ...
                            function distance(order_id, addr) {
                                $.post("<?php echo U('ele/distance');?>", {order_id: order_id, addr: addr}, function (data) {
                                    $('#distance').html(data);
                                });
                            }
                        </script> 
                ...
                <div class="order_plus table1" style="display: none;">
                    <div class="order_style2">
                        ...
                        <span class="left"> <img src="__TMPL__statics/images/tp_12.png">到店后请出示抢购卷密码给商家，确认领取优惠再付现金</span>
                        <div class="clear"></div>
                    </div>
                </div>
                <div class="order_p1">
                    <div class="order_s3"> <p>支付：<span class="order_s4">$<?php echo round($order['total_price']/100,2);?></span></p>
                        <div id="logistics">
                        <?php if(!empty($order['logistics'])): ?><p>包含送餐费：<code><span class="order_s4">$<?php echo round($order['logistics']/100,2);?></span></code></p>
                        <?php else: ?>
                            <p>请到店消费，如需打包带走，请告知服务员。</p><?php endif; ?>
                        </div>
                    </div>
                    <div> 
   ...
   <style> .admin {border: 1px rgba(244, 244, 244, 0.99) solid;}</style>    
       <!--PC绑定手机开始--> 
         <div class="clear"></div>
                <?php if(!empty($MEMBER['mobile'])): ?><div class="admin admin-two"> 将发送抢购卷密码至手机号：<font class="fontcl2"><?php echo ($MEMBER['mobile']); ?></font> <a id="mobile_change" href="javascript:void(0);">更换手机号</a> </div>
                    <?php else: ?>
                    <div class="admin admin-two"> 您还未绑定手机号，<a style="padding-left: 0px;" id="mobile_bind" href="javascript:void(0);">立即绑定</a> </div><?php endif; ?>
                <div class="clear"></div>
            ...
            <script>
                $(document).ready(function () {
                    $("#ele_btn").click(function () {
                        var url = "<?php echo U('ele/pay2',array('order_id'=>$order['order_id']));?>";
                    ...
                    <?php if(is_array($ordergoods)): foreach($ordergoods as $key=>$item): ?><tr>
                            <td>
                                <div class="tab_nr">
                                    ...
                                    <img src="<?php echo config_img($products[$item['product_id']]['photo']);?>" width="134" height="85" class="img_3"></div>
                                    <div class="left tab_wz">
                                        <p class="tab_nr1"><?php echo ($products[$item['product_id']]['product_name']); ?></p>
                                    </div>
                                </div>
                            </td>
                            <td width="115">
                                <div class="tab_price">$<span class="price"><?php echo round($products[$item['product_id']]['price']/100,2);?></span></div>
                            </td>
                            <td width="116"> <?php echo ($item['num']); ?> </td>
                            <td><?php echo round($item['total_price']/100,2);?></td>
...
<div class="order-cover" id="addr_add">
    <div class="add-newdress">
        ...
        <form id="address"  method="post">
            <input type="hidden" class="addrs_id" name="addrs_id" value="<?php echo ($order["addr_id"]); ?>">
            <div class="add-message">
                <p><span>*</span> 联系人：
                    <input type="text" value="<?php if($MEMBER["nickname"] != null): echo ($MEMBER["nickname"]); else: endif; ?>" name="name" class="add-text">
                </p>
                <p style="display: none"><span>*</span> 地区：
                    <input name="city_name" id="administrative_area_level_1" style="width: 100px; margin-left: 28px;" class="add-text">
                    <input name="area_name" id="locality" style="width: 100px;" class="add-text">
                    <input name="business_name" id="sublocality_level_1" style="width: 100px;" class="add-text">
                </p>
                <p><span>*</span> 手机号码：
                    <input type="text" value="<?php if($MEMBER["mobile"] != null): echo ($MEMBER["mobile"]); else: endif; ?>" name="mobile" class="add-text2">
                </p>
                <p><span>*</span> 收货地址：
                    <input type="text" id="addr" value="" name="addr" class="add-text2 add-dw">
                </p>
            </div>
    ...
    <div class="add-newdress">
        ...
        <form id="addredit"  method="post">
            ...
            <div class="add-message">
                ...
                <p style="display: none"><span>*</span> 地区：
                    <input name="city_name" id="eadministrative_area_level_1" style="width: 100px; margin-left: 28px;" class="add-text" value="<?php echo ($city["name"]); ?>">
                    <input name="area_name" id="elocality" style="width: 100px;" class="add-text" value="<?php echo ($area["area_name"]); ?>">
                    <input name="business_name" id="esublocality_level_1" style="width: 100px;" class="add-text" value="<?php echo ($business["business_name"]); ?>">
                </p>
                <p><span>*</span> 手机号码：
    ...
    $(document).ready(function () {
        ...
        if (user_id == '' || user_id == null) {
            ...
            if (mobile == '' || mobile == null) {
                check_user_mobile_for_pc('<?php echo U("mobile/tuan/tuan_sendsms");?>', '<?php echo U("mobile/tuan/tuan_mobile");?>');
            }
            $("#mobile_bind").click(function () {
                check_user_mobile_for_pc('<?php echo U("mobile/tuan/tuan_sendsms");?>', '<?php echo U("mobile/tuan/tuan_mobile");?>');
            });
            $("#mobile_change").click(function () {
                change_user_mobile_for_pc('<?php echo U("mobile/tuan/tuan_sendsms");?>', '<?php echo U("mobile/tuan/tuan_mobile2");?>');
            });
        }
    })
</script>
...
<script>
    ...
    function initAutocomplete() {
        ...
        $(document).on('click', '.order-xg', function () {
            autocomplete = new google.maps.places.Autocomplete((document.getElementById('addrs')),{types: ['geocode']});
    ...
    function fillInAddress2() {
        var place = autocomplete.getPlace();
        document.getElementById("esublocality_level_1").value = "";
        document.getElementById('elocality').value = "";
        document.getElementById('eadministrative_area_level_1').value = "";
        for (var i = 0; i < place.address_components.length; i++) {
            var addressType = place.address_components[i].types[0];
            if (componentForm[addressType]) {
                document.getElementById('e'+addressType).value = place.address_components[i][componentForm[addressType]];
            }
        }
    }
</script>
