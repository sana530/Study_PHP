...
<div class="sjgl_lead">
    <ul>
        <li><a href="#">商家管理</a> > <a href="">Mall</a> > <a>待确认商品</a></li>
    </ul>
</div>
...
<?php if(is_array($list)): foreach($list as $key=>$order): ?><table  class="tuan_table1"  width="100%" border="0">
  <tr class="tr_left_2">
    ...
    订单ID:<span class="mallorder_jg"><?php echo ($order["order_id"]); ?></span>
        ...
        <span class="td_top_1"> <?php if(!empty($order['addr_id'])): ?>收货地址:
  ...
  <tr>
    ...
    <td class="left6" width="9%"><?php echo ($users[$order['user_id']]['account']); ?>
    ...
    <td class="left7" width="8%">
        ...
        <?php if($order['status'] == 3): ?><a class="radius3 btn" mini='confirm' href="<?php echo U('order/distribution',array('order_id'=>$order['order_id']));?>">确认订单</a><br/>
        <?php else: ?>
             等待客户确认<br/><?php endif; ?>
