...
<div id="login-input" class="life-infor-float" style="display:none;">
	<div class="left"><span>经度：</span><input type="text" name="data[lng]"  value="<?php echo (($SHOP["lng"])?($SHOP["lng"]):''); ?>" id="data_lng"/></div>
	<div class="right"><span>纬度：</span><input type="text" name="data[lat]"  value="<?php echo (($SHOP["lat"])?($SHOP["lat"]):''); ?>"  id="data_lat"/></div>
</div>
<div class="row">
	<div class="line">
		<span class="x3">店铺地址：</span>
		<span class="x9">
			<input id="addr" type="text" class="text-input" name="data[addr]" value="<?php echo (($SHOP["addr"])?($SHOP["addr"]):''); ?>" />
		</span>
	</div>
	<script>
        var autocomplete;
